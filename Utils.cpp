...
{
  bool returnValue = false; // default is that the file does not exist
  std::ifstream inStream;

  std::cout << TAG << "test existence of input file '" << fileName << "'\n";
...
{
  bool returnValue = true; // default is that the file does not exist
  std::ifstream inStream;

  std::cout << TAG << "test nonexistence of input file '" << fileName << "'\n";
  inStream.open(fileName.c_str());
  if(inStream.fail())
  {
    std::cout << TAG << "file '" << fileName << "' does not exist\n";
    returnValue = true;
    inStream.close();
  }
  else
  {
    std::cout << TAG << "file '" << fileName << "' does indeed exist\n";
    returnValue = false;
    inStream.close();
  }
...
{
  double timeNew = 0.0;
  return Utils::timecall(timestring, timeNew);
}
